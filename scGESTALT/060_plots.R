...
alignment_permutation <- read_tsv("./data/020.2_alignment_threshold.txt")
...
aligned_reads <- read_tsv("./data/020.1_alignment/SRR6176748.txt")
...
pos_example_seq <- filter(aligned_reads,score>160,score<161)
...
neg_example_seq <- filter(aligned_reads,score>40,score<41)
...
aligned_reads <-mutate(aligned_reads,UB=substr(seq,1,10))
CB <- lapply(aligned_reads$description,get_CB)
aligned_reads$CB <- unlist(CB)
